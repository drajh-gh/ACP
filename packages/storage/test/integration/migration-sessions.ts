...
try {
  ...
  assert.equal(applied.applied.length,16); assert.equal(applied.alreadyApplied.length,0);
  ...
  checks++; process.stdout.write("PASS migration session: all 16 actual migrations apply, checksum-replay and reverse on an empty database\n");
