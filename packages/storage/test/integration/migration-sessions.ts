...
try {
  ...
  assert.equal(applied.applied.length,19); assert.equal(applied.alreadyApplied.length,0);
  ...
  checks++; process.stdout.write("PASS migration session: all 19 actual migrations apply, checksum-replay and reverse on an empty database\n");
