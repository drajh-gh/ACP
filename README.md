...
- a one-snapshot recorded request-history read exposed through authenticated
  `get_request_history`, with exact project/request identity, complete bounded
  associations and explicit non-authority; no browser writer, producer metadata,
  current lifecycle inference or partial-history success
